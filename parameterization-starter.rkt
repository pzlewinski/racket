...
(define (positive-only lon)(return-specific-list positive? lon))
...
(define (negative-only lon) (return-specific-list negative? lon))

(define (return-specific-list fn lon)
  (cond [(empty? lon) empty]
        [else
         (if (fn (first lon))
             (cons (first lon)
                   (return-specific-list fn (rest lon)))
             (return-specific-list fn (rest lon)))]))
